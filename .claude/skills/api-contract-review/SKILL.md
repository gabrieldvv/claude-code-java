...
| Field | Content |
|---|---|
| **Severity** | high / medium / low — based on consumer and compatibility impact (a breaking contract change outranks an inconsistency, which outranks a cosmetic issue) |
| **Confidence** | high / medium / low — add **Assumptions** when medium or low |
| **Location** | endpoint / field, or `file:line` |
...
- [ ] Would any recommendation break a published API contract or change behavior for existing consumers? If so, is that flagged?
