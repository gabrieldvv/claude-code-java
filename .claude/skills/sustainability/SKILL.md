---
name: sustainability
description: Perform a green software / sustainability audit of code or architecture using five patterns from Chapter 8 (Lean Packaging, Lean Storage, Lean Communication, Efficient Execution, Memory Efficiency). Trigger on explicit requests for a sustainability audit, green review, or efficiency audit. This is the broad resource-waste audit (CPU, memory, storage, network, package size); do NOT trigger on general architecture reviews, code quality reviews, or refactoring requests (use architecture-review), on code-level Java micro-performance smells like streams/boxing/regex (use performance-smell-detection), or on JPA/database performance (use jpa-patterns).
---
